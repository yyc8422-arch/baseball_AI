...
AI-Server/
  ...
  app/
    ...
    routers/video.py             POST /api/analysis (업로드), GET /api/analysis/{video_id} (상태/결과 조회)
    services/video_storage.py    대용량 파일을 안전하게 저장하는 핵심 로직
    services/analysis_store.py   분석 상태/결과 레코드 저장·조회 (지금은 JSON 파일, 나중에 DB 로 교체)
  ...
  storage/
    ...
    results/                     분석 레코드 JSON (<video_id>.json, status: queued/processing/done/failed)
```

## 업로드 흐름

1. 프론트엔드(`web/Frontend/js/analysis.js`)가 `POST /api/analysis` 로 `video` 파일 +
...
4. 저장이 끝나면 `status="queued"` 레코드를 만들고, 그 **뒤에만**
   `BackgroundTasks.add_task(process_video, ...)` 로 분석을 큐에 올린 다음 바로 응답합니다.
5. 프론트엔드는 응답의 `video_id` 로 `GET /api/analysis/{video_id}` 를 2초마다 조회하고,
   `processing` → `done`(검출 통계 `summary`) / `failed`(실패 이유 `error`)를 화면에 표시합니다.
   프레임별 관절 좌표는 용량이 커서 `?include_pose=true` 일 때만 내려줍니다.

## 앞으로 채워야 할 곳

- 자세 분석 로직 — `process_video()` 가 뽑은 프레임별 관절 좌표로 각도/개선 포인트 계산.
- 분석 레코드를 DB로 옮기기 — 지금은 `storage/results/*.json` 파일로 저장합니다.
  Spring 백엔드와 같은 MySQL을 공유할지 정한 뒤 `analysis_store.py` 안쪽만 바꾸면 됩니다.
